...
# --- System Prompt ---
SYSTEM_PROMPT = """
You are the Procurement & Funding Agent, a specialized module of the StartupX Startup Intelligence Engine.
Role: Funding Readiness + Vendor Procurement + Resource Allocation.

Responsibilities:
- Build a roadmap for investor readiness (Pitch deck, Financials, etc.).
- Source and vet technology/service vendors.
- Calculate initial burn rate and budget requirements.
- Identify grants and government funding schemes.
- Optimize procurement costs.
- Advise on equity split and cap table structure.
- Generate a hiring plan for core team members.
...
Behavior Rules:
- Be financially prudent, strategic, and resourceful.
- Focus on investor appeal and operational efficiency.
"""

# --- Agent Initialization ---
agent_config = AgentConfig(
    name="StartupX Procurement Agent",
    description="Specialized in Funding Readiness and Procurement Strategy.",
    capabilities={"ai": ["procurement", "funding_readiness"]},
    webhook_host="0.0.0.0",
    webhook_port=5003,
    registry_url=REGISTRY_URL,
    api_key=ZYND_API_KEY,
    config_dir=".agent-procurement",
    price=None
)
...
def handle_message(message: AgentMessage, topic: str):
    ...
    print(f"[Procurement Agent] Sourcing resources for: {message.content[:50]}... (Session: {session_id})")
    ...
    # Get history
    history_data = get_history(session_id, "procurement")
    formatted_history = []
    ...
    try:
        ...
        # Save human message
        save_message(session_id, "procurement", "human", message.content)
        ...
        # Save AI message
        save_message(session_id, "procurement", "ai", res_content)
        
        agent.set_response(message.message_id, res_content)
        print(f"[Procurement Agent] Response sent for {message.message_id}")
...
if __name__ == "__main__":
    print(f"🚀 StartupX Procurement Agent Running on port 5003")
    while True:
        time.sleep(1)
